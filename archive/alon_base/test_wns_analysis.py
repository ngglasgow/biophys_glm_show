...
home_dir = '/Users/nate/ngglasgow@gmail.com/Data_Urban/NEURON/AlmogAndKorngreen2014/par_ModCell_5_thrdsafe'
data_dir = 'test_wns_400_output'
...
time = pd.DataFrame(np.arange(0, 3200., 0.1))
...
for file in data_files:
    if 'i' in file:
        ...
        tneost_list = []
        for i in range(1, 51):
            tst = binary_time.iloc[6500:, [0, i]]
            tst = tst.iloc[:, 0][tst.iloc[:, 1] > 0]
            tst_list.append(tst)
            tneost = SpikeTrain(times=tst.values*pq.ms, t_start=650., t_stop=3200)
...
test = vm_list[5].iloc[:, :4]
test2 = vm_list[6].iloc[:, :4]
test3 = vm_list[9].iloc[:, :4]
plt.figure()
plt.plot(test)
plt.figure()
plt.plot(test2)
