...
wns_file
mean_noise = wns_file.mean(axis=1)
time = np.arange(0, 3200.0, 0.1)
vm = channels['iH']['1']['Vm']
len(time)
len(mean_noise)
fig, ax = plt.subplots()
ax.plot(time, vm.iloc[:, 0], 'k', linewidth=0.5)
ax.set_xlabel('Time (ms)')
ax.set_ylabel('Voltage (mV)')
ax.plot(mean_noise, 'k')
fig
fig.savefig(figure_dir + '1vm.png', dpi=300, format='png')
...
sns.set_style('ticks')
scale_list = ['0.5', '1', '1.5', '1.2']
for channel in channels:
    plt.figure()
    plt.title(channel)
    for scale in scale_list:
        vm = channels[channel][scale]['Vm']
        vm_trial_00 = vm.iloc[:, 0]
...
for channel in channels:
    ...
    for scale in sorted_scales:
        plt.figure()
        plt.title('Spike Times of ' +  channel + '*' + scale)
        plt.xlabel('Time (ms)')
        plt.ylim(0, 101)
...
for channel in channels:
    ...
    for scale in sorted_scales:
        ...
        for i in range(len(spiketimes.columns)):
            ...
            neo_spiketrain = SpikeTrain(times=spiketrain.values*pq.ms, t_start=0., t_stop=3200.)
            neo_spiketimes.append(neo_spiketrain)

        # convert neo spiketimes to binned spiketrains with x ms bins
        bins = 1.0      # 1 ms bins for 1 kHz final
        binned_spiketrains = elephant.conversion.BinnedSpikeTrain(neo_spiketimes, binsize=bins*pq.ms, t_start=0.*pq.ms, t_stop= 3200.*pq.ms)
        binned_spiketrains = pd.DataFrame(binned_spiketrains.to_array()).T
...
# save to file as non scaled or scaled
fig.savefig(figure_dir + 'alon_wns_vm_raster_psth_3nonoverlay_matchheight.png', dpi=300, format='png')
fig.savefig(figure_dir + 'alon_wns_vm_raster_psth_3nonoverlay_matchheight_scaled.png', dpi=300, format='png')

vm_lims = []
psth_lims = []
for col, channel in zip(range(8), channels):
...
fig, axs = plt.subplots(4, 8, figsize=(24, 4.5), gridspec_kw=gs_kw, constrained_layout=True)
...
for col, channel in zip(range(8), channels):
    ...
    for i in range(len(scale_list)):
        ...
        #axs[1, col].set_ylim(-71, 41)
        axs[1, col].set_xlim(14500, 16500)
        axs[1, col].set_ylim(-86, 26)
    # axs[1].clear()
    ...
    # axs[3, col].set_ylim(-8, 168)
    axs[3, col].set_xlim(1455, 1655)
    axs[3, col].set_ylim(-10, 192)

fig.savefig(figure_dir + 'alon_wns_vm_raster_psth_3overlay_scaled.png', dpi=300, format='png')
...
\need to work on plots, but will be similar to STA feature plots below
...
axs[1, 1].set_ylabel('Scaling Factor (fold-change)')

fig.savefig(figure_dir + 'all_stats.png', dpi=300, format='png')
...
fig.savefig(figure_dir + 'mod_stats.png', dpi=300, format='png')
...
axs[1, 0].set_ylabel('Current (pA)')
axs[1, 1].set_ylabel('Scaling Factor (fold-change)')
fig.savefig(figure_dir + 'nak_stats.png', dpi=300, format='png')
...
for i in range(len(wns_file.columns)):
    wns = wns_file.iloc[:, i]
    wns_mean = np.mean(wns.iloc[1500:31501])
...
for ax, channel, k in zip(axs.flat, ordered_channels, range(8)):
    ...
    for i in range(n_scales):
        ...
        ax.plot(time, sta, label=scale, color=redblue10[i], linewidth=2)
...
fig
# make adjustmens to minor ticks for unscaled version
...
fig
# save fig to file
fig.savefig(figure_dir + 'STAs_columns_ordered_scaled_ylabel_cutoff.png', dpi=300, format='png')
fig.savefig(figure_dir + 'STAs_columns_ordered_scaled_labeled.png', dpi=300, format='png')
...
fig.savefig(figure_dir + 'all_features.png', dpi=300, format='png')
...
axs[1, 1].legend(loc='center left', bbox_to_anchor=(1, 0.5))
fig.savefig(figure_dir + 'mod_features.png', dpi=300, format='png')
...
axs[1, 1].legend(loc='center left', bbox_to_anchor=(1, 0.5))
fig.savefig(figure_dir + 'nak_features.png', dpi=300, format='png')
...
sns.palplot(PuOr11_k)
sns.palplot(redmin1_bluemin1)

'''####################### tests for sta feature selector ##################'''
sta_features = pd.read_csv(table_dir + 'sta_features_index.csv')
n_scales = 10
time = np.linspace(-35, 0.0, num=351)
stas = sta_mean.iloc[:, 3:-50] * 1000
