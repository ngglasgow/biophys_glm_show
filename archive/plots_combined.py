...
gs_kw = dict(height_ratios=[1, 0.3, 1, 1, 0.3])
fig, axs = plt.subplots(figsize=(6.6, 8), nrows=5, ncols=2, gridspec_kw=gs_kw, constrained_layout=True)
...
# do weights here
axs[3, 0].plot(slopes_0.stim_weights)
axs[3, 1].plot(slopes_opt.stim_weights)

axs[3, 0].set_ylabel('Coefficient Values')
axs[3, 0].set_title('\u03BB = 0')
axs[3, 1].set_title('\u03BB = *')
...
axs[4, 0].scatter(x_spread, y_spread, c=lambda_0, cmap=stim_cmap, norm=stim_norm)
axs[4, 1].scatter(x_spread, y_spread, c=lambda_opt, cmap=stim_cmap, norm=stim_norm)


cb = fig.colorbar(cm.ScalarMappable(stim_norm, stim_cmap), ax=axs[4, :], orientation='horizontal', aspect=30, shrink=0.5)
...
tax[0].scatter(x_spread, y_spread, c=lambda_opt, cmap=stim_cmap, norm=stim_norm)
tax[1].set_axis_off()
tcb = test.colorbar(cm.ScalarMappable(stim_norm, stim_cmap), pad=1)