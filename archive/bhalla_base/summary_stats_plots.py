...
for row, stat in zip(range(3), stat_list):
    ...
    for channel, i in zip(important_channels, range(3)):
        ...
        axs[row, 0].set_xticks([0, 1, 2, 3])
        axs[row, 0].set_xlim(-0.2, 3.2)
...
for row, stat in zip(range(3), stat_list):
    ...
    for channel, i in zip(na_k_channels, range(3)):
        ...
        axs[row, 1].set_xticks([0, 1, 2, 3])
        axs[row, 1].set_xlim(-0.2, 3.2)
...
axs[0, 0].legend(loc='lower center', bbox_to_anchor=(0.4, 1.0), ncol=2)
axs[0, 1].legend(loc='lower center', bbox_to_anchor=(0.4, 1.0), ncol=2)
fig

# save fig to file
fig.savefig(figure_dir + 'stats_summary_label3.png', dpi=300, format='png')
