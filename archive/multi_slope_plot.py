...
fig, axs = plt.subplots(nrows=2, ncols=1)
axs[0].scatter(xx, yy, c=stim_slopes, cmap=stim_cmap)
axs[1].scatter(xx, yy, c=hist_slopes, cmap=hist_cmap)
fig.suptitle(cmap.name + ' | vmin={}:vmax={}'.format(vmin, vmax))
...
# transposed, clipped
fig, axs = plt.subplots(nrows=1, ncols=2)
axs[0].scatter(xst, yst, c=stim_slopes.T, cmap=stim_cmap, norm=stim_norm)
axs[1].scatter(xst, yst, c=hist_slopes.T, cmap=hist_cmap, norm=hist_norm)
axs[0].set_ylim(23, -1)
axs[1].set_ylim(23, -1)
fig.suptitle(cmap.name + ' | vmin={}:vmax={}'.format(vmin, vmax))
fig.savefig(os.path.join(figure_dir, 'kA_stim_hist_lambdas.png'), dpi=300, format='png')
